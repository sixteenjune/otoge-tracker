...
export type Filters = { query: string; difficulty: string; level: string; constant: string; bpmMin: string; bpmMax: string; version: string; genre: string; intl: string; deleted: boolean; sort: string; direction: "asc"|"desc" };
export const emptyFilters: Filters = { query:"", difficulty:"all", level:"all", constant:"all", bpmMin:"", bpmMax:"", version:"all", genre:"all", intl:"all", deleted:false, sort:"title", direction:"asc" };
export function filterSongs(songs: NormalizedSong[], f: Filters) {
  const q=f.query.trim().toLocaleLowerCase(); const out=songs.filter(s => {
    ...
    const constant = f.constant === "all" || s.charts.some(c => c.constant !== undefined && (f.constant === "10+" ? c.constant >= 10 : c.constant === Number(f.constant)));
    ...
    return (!q || s.searchText.includes(q)) && hasDiff && hasLevel && constant && bpm && (f.version==="all" || s.version?.ja===f.version) && (f.genre==="all" || s.genre?.ja===f.genre) && (f.intl==="all" || (f.intl==="yes" ? s.isInternational : !s.isInternational)) && (f.deleted ? s.isDeleted : !s.isDeleted);
  });
