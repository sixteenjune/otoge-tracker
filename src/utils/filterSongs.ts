...
export function filterSongs(songs: NormalizedSong[], f: Filters) {
  ...
  const value=(s:NormalizedSong): string|number => f.sort==="title" ? s.title.ja||"" : f.sort==="artist" ? s.artist?.ja||"" : f.sort==="version" ? s.version?.ja||"" : f.sort==="genre" ? s.genre?.ja||"" : f.sort==="bpm" ? s.bpm||0 : f.sort==="date" ? s.addedDate||"" : f.sort==="notes" ? Math.max(...s.charts.map(c=>c.notes||0)) : Math.max(...s.charts.map(c=>Number.parseFloat(c.level || "0")||0));
